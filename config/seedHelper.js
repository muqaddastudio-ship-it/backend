...
const sampleProducts = [
  {
    ...
    variants: [
      ...
      { size: "M", color: "Noir Black", colorHex: "#000000", stock: 4 },
      { size: "L", color: "Noir Black", colorHex: "#000000", stock: 10 },
      { size: "XL", color: "Noir Black", colorHex: "#000000", stock: 2 }
    ],
    status: "active",
    featured: true,
    ratingAvg: 4.9,
  ...
  },
  {
    ...
    variants: [
      ...
      { size: "XL", color: "Ivory White", colorHex: "#ffffff", stock: 0 }
    ],
    status: "active",
    featured: true,
    ratingAvg: 4.8,
...
const seedHelper = async () => {
  ...
  const existingCustomer = await User.findOne({ email: "user@muqaddasstudio.com" });
  if (!existingCustomer) {
    await User.create({
      name: "Ayesha Khan",
  ...
  console.log('[Seed Helper] Initialized default database state.');
};
